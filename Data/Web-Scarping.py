...
class ArticleScraper:
    ...
    def fetch_article(self, url: str, article_limit_reached: List[bool]) -> Optional[Article]:
        ...
        published_time = metadata.get('datePublished') or self.extract_date_from_html(soup, 'published_time') or '2024-01-01T00:00:00Z'
        last_updated = metadata.get('dateModified') or self.extract_date_from_html(soup, 'last_updated') or published_time
        author = metadata.get('author') or 'Default Author'
        word_count = int(metadata.get('wordCount') or len(self.extract_full_text(soup).split()))
        video_duration = metadata.get('video') or None
